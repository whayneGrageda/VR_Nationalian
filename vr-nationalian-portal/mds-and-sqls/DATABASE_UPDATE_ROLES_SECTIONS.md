# Database Update: User Roles & Sections

## Overview
This update adds user roles (student, prof, admin) and sections management for professors.
...
```sql
-- ================================================================
--  DATABASE UPDATE: Add User Roles (Normalized)
...
ADD COLUMN role_id INT DEFAULT 1 REFERENCES tblroles(role_id);
...
```sql
-- Update fn_login to return role information
...
-- ================================================================
--  SECTIONS TABLE
--  Professors can create sections/classes for students
-- ================================================================
...
CREATE INDEX idx_sections_professor ON tblsections(professor_id);

-- Auto-update timestamp
CREATE TRIGGER trg_update_section_timestamp
    BEFORE UPDATE ON tblsections
    FOR EACH ROW EXECUTE FUNCTION fn_update_timestamp();
```

### Step 2: Update Users Table for Section Assignment

```sql
-- Add section_id to tblusers (students belong to one section)
ALTER TABLE tblusers 
ADD COLUMN section_id UUID REFERENCES tblsections(section_id) ON DELETE SET NULL;

-- Index for faster queries
CREATE INDEX idx_users_section ON tblusers(section_id);
```

### Step 3: Section Management Functions

```sql
-- ================================================================
--  FUNCTION: Create Section (Simple - just name)
-- ================================================================
...
-- ================================================================
--  FUNCTION: Get Professor's Sections
-- ================================================================
...
-- ================================================================
--  FUNCTION: Get Students in Section
-- ================================================================
...
-- ================================================================
--  FUNCTION: Create Student Account (by Professor)
-- ================================================================
...
WHERE table_schema = 'public' AND table_name LIKE 'tblsection%';
...
### Create a Section (Simple - just name)
```sql
SELECT fn_create_section(
    '<professor_user_id>',
...
```sql
SELECT fn_get_professor_sections('<professor_user_id>');
```

### Get Students in a Section
```sql
SELECT fn_get_section_students(
    '<section_id>',
    '<professor_user_id>'
);
```
...
### Hide a Section
```sql
SELECT fn_toggle_section_visibility(
    '<section_id>',
    '<professor_user_id>',
    TRUE
);
...
**1. Sections Page - List View**
```javascript
// Get all sections
const sections = await fetch('/rest/v1/rpc/fn_get_professor_sections', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({ p_professor_id: userId })
});
```

**2. Add Section - Pop-up Form**
```javascript
// Create section (just name field)
await fetch('/rest/v1/rpc/fn_create_section', {
  method: 'POST',
  body: JSON.stringify({
    p_professor_id: userId,
    p_section_name: 'CS 101'
  })
});
```

**3. Click Section - Opens Students Page**
```javascript
// Get students in section
const students = await fetch('/rest/v1/rpc/fn_get_section_students', {
  method: 'POST',
  body: JSON.stringify({
...
**4. Create Student Account - In Section Page**
```javascript
// Create student
await fetch('/rest/v1/rpc/fn_create_student', {
  method: 'POST',
  body: JSON.stringify({
    p_professor_id: userId,
    p_section_id: sectionId,
...
```javascript
// Update student
await fetch('/rest/v1/rpc/fn_update_student', {
  method: 'POST',
  body: JSON.stringify({
    p_professor_id: userId,
...
```javascript
// Delete student
await fetch('/rest/v1/rpc/fn_delete_student', {
  method: 'POST',
  body: JSON.stringify({
    p_professor_id: userId,
    p_student_id: studentId
...
|---------|-----------|--------|
| **1** | student | Mobile app only, gameplay, view own progress |
| **2** | prof | Web only, manage sections, view student progress |
| **3** | admin | Web only, full CRUD on all data |

---

Created for NU Quest - Database Schema v2.0
